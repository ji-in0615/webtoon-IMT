...
def list_files(in_path): 
    ...
    gt_files = []
    name_files = []
    for (dirpath, dirnames, filenames) in tqdm(os.walk(in_path)):
...
def saveText(save_to=None, text=None, name=None):
    ...
    with open(text_file, 'w') as f:
        for i, box in tqdm(enumerate(text)):
            if box == []: continue
...
def saveAllImages(save_to=None, imgs=None, index1=None, ext=None):
    ...
    if index1 is not None:
        for idx2, img in tqdm(enumerate(imgs)):
            time.sleep(0.1)
            cv2.imwrite(save_to + index1 + '_' + str(idx2) + ext, img)
    else:
        for idx1, img in tqdm(enumerate(imgs)):
            time.sleep(0.1)
...
def loadJson(json_file):
    ...
    with open(json_file) as data_file:
        ...
        for idx, info in tqdm(enumerate(data['shapes'])):
            time.sleep(0.1)
...
def loadText(txt_file):
    arr = []
    with codecs.open(txt_file, encoding='utf-8') as file:
...
def loadSpacingWordInfo(load_from=None):
    arr_list = list()
    length = 0
    with codecs.open(load_from, encoding='utf-8') as file:
        coordinate = file.readlines()
        for line in tqdm(coordinate):
            time.sleep(0.1)
            tmp = line.split(',')
            tmp[-1] = tmp[-1].strip('\r\n')
            if tmp[0] == '': continue
